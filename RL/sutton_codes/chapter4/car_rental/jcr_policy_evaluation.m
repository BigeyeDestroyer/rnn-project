...
% JCR_POLICY_EVALUATION - Performs policy evaluation 
% returning the state-value function for the jacks car rental example.
% 
...
% 
% Input: 
%     V      : (n_max + 1, n_max + 1), array to hold the values 
%              of the state-value function 
%     pol_pi : (n_max + 1, n_max + 1), action at each state
%     gamma  : discount factor
%     Ra     : (n_max + n_trans_max + 1, ), reward in the morning
%     Pa     : (n_max + n_trans_max + 1, n_max + 1), probabilities 
%              from morning to night 
%     Rb     : similar for LOC_b
%     Pb     : similar for LOC_b
%
%     max_num_cars_can_tranfer : maximum num of transfer 
%
% Outputs
%     V : (n_max + 1, n_max + 1), the updated state-value 
...
MAX_N_ITERS = 100; % loops for evaluation
iterCnt = 0; 
...
while( (delta > CONV_TOL) && (iterCnt <= MAX_N_ITERS) ) 
    ...
    for si = 1: nStates, 
        ...
        %---
        tic;
        V(na1, nb1) = jcr_rhs_state_value_bellman(na, nb, ntrans, V, gamma, ...
