...
% 
% Inputs: 
%     pol_pi : (n_max + 1, n_max + 1), array to hold the actions
%     V      : (n_max + 1, n_max + 1), an array holding the state-values
% 
% Outputs:
%     pol_pi       : the updated policy
%     policyStable : bool to determine whether the policy is stable
...
for si=1: nStates
    ...
    posActionsInState = -posB: posA; 
    npa = length(posActionsInState); 
    Q = -Inf * ones(1, npa);
    tic; 
    for ti = 1: npa,
        ...
        %---
        Q(ti) = jcr_rhs_state_value_bellman(na, nb, ntrans, V, gamma, ...
