...
% 'epsilon' in the epsilon-soft policy: 
%  eps = 0 => gready
%  eps = 1 => random
eps = 0.1; 

% fixed step size learning parameter:
% alpha = 0.1;
% alpha = 0.05; 
...
% soft policy, and the initial policy is random
pol_pi = 0.5 * ones(nStates, nActions);

% Following two terms are for 'Q' estimation
% firstSARewSum : 'reward' for the first state seen accumulated
% firstSARewCnt : 'count' for the first state seen accumulated
...
for hi = 1: N_HANDS_TO_PLAY
    ...
    % Since we use soft-policy, we start according to the 'policy_pi'
...
% Actually, 'mc_value_fn' here is V^{star}
mc_value_fn = max(Q, [], 2);
...
title('a usable ace'); 
drawnow; 
% fn = sprintf('bj_opt_pol_ua_%d_image.eps', N_HANDS_TO_PLAY); 
% saveas(gcf, fn, 'eps2'); 