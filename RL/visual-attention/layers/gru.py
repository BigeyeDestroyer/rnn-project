...
class GRULayer(object):
    def __init__(self, rng, layer_id, shape, X, mask,
        ...
        self.Uz = theano.shared(value=ortho_weight(ndim=self.hid_size),
                                name=prefix + '#Uz')
        ...
        self.Ur = theano.shared(value=ortho_weight(ndim=self.hid_size),
                                name=prefix + '#Ur')
        ...
        self.Uh = theano.shared(value=ortho_weight(ndim=self.hid_size),
                                name=prefix + '#Uh')
        ...
        # This is the regular 'h' when we start with zero
        h, _ = theano.scan(fn=_step, sequences=[self.X, self.mask],
...
class BdGRULayer(object):
    # Bidirectional GRU Layer
    def __init__(self, rng, layer_id, shape, X, mask,
                 use_noise=1, batch_size=1, p=0.5):
        fwd = GRULayer(rng, '_fwd_' + layer_id, shape, X, mask, use_noise, batch_size, p)
        bwd = GRULayer(rng, '_bwd_' + layer_id, shape, X[::-1], mask[::-1], use_noise, batch_size, p)
